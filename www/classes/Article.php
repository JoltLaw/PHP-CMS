...
class Article {
   ...
   * 
   * @param object $conn Connection to the detabase
   * @param interger $limit Number of records returned
 ...
 */
  ...
  /**
 * Validate properties on the form. putting any validation erros inside the errors property on this object
 * 
 * @return boolean true if properties are valid, else false
 */
...
protected function validate() {
  ...
  if ($this->published_at != "") {
    ...
    if($date_time === false) {
      $this->errors[] = "Invalid date, and or time";
    } else {
      $date_errors = date_get_last_errors();

      if($date_errors["warning_count"] > 0) {
        $this->errors[] = "Invalid time";
      }
    }
  }
 ...
 * @param object $conn Connection to the database
 * 
 * @return boolean true if the delete was successful, otherwise false
 */
...
/**
 * Insert a new article with its current values
 * 
 * @param object $conn Connection to the database
 * 
 * @return boolean true if it was able to insert the doccument, otherwise false
 */
 ...
 * @param object $conn Connection to the database
 * 
 * @param string $filename The filename of the iamge file
 * 
...
/** 
 * publish article, setting published_at to current time and date
 * 
 * @param object $conn Connection to the database
 * 
